    ...
    public class GifImage : System.Windows.Controls.UserControl
    {
        ...
        private void DeletePreviousImage()
        {
            ...
            if (gifAnimation != null)
            {
                ...
                // Jack Zhu  2014/7/31 : Can't let gifAnimation be null, otherwise the old animation can't be stop!
                // gifAnimation = null;
            }
        }
        ...
        {
            // Jack Zhu  2014/7/31 : Can't create a new object, otherwise the old animation can't be stop!
            if (gifAnimation == null)
            {
        ...
        private void CreateFromSourceString(string source)
        {
            ...
            if (source.Trim().ToUpper().EndsWith(".GIF") || ForceGifAnim)
            {
                ...
                else
                {
                    ...
                    else if (leftPart == "file://") // zhujie 2014/7/31 : file:// can't load one gif after another by using GetGifStreamFromHttp Method
                    {
                        GetGetGifStreamFromDisk(uri);
                    }
        ...
        // zhujie 2014/7/31 : file:// can't load one gif after another by using GetGifStreamFromHttp Method
        private void GetGetGifStreamFromDisk(Uri uri)
        {
            try
            {
                var strPath = uri.ToString().Substring(8);
                var streamInfo = new FileStream(strPath, FileMode.Open);
                ReadGifStreamSynch(streamInfo);
            }
            catch (Exception exp)
            {
                RaiseImageFailedEvent(exp);
